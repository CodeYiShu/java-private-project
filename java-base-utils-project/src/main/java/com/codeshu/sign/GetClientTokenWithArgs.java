 ...
 */
public class GetClientTokenWithArgs {

	public static void main(String[] args) {
		new GetClientTokenWithArgs().auth();
	}
	...
	public void auth() {
		//指定密钥
		String token = "{'so_import':'511ae999a65054185698b889ab940e10'}";
		//将配置文件中的密钥，转为JSON对象
		JSONObject tokenJson = JSONUtil.parseObj(token);
		//模拟请求参数
		int t = (int) (new DateTime().millisecond() / 1000);  //时间戳
		String appKey = "so_import";  //appkey
		String data = "1,2,3".replace("\"", ""); //普通参数
	...
	public static String getSign(String secret, Map<String, Object> map) {
		...
		String info = secret + str.append(secret).toString();  //老版——需要前后加上secret
	...
	private static String getMd5(String input) {
		...
		try {
			...
			//老版——需要对,进行编码
			String encode = URLEncoder.encode(input, "utf-8").replace("+", "%20").replace("*", "%2A").replace("%7E", "~").replace("%2C", ",");
